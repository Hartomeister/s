...
async def send_mes(message: types.Message):
	...
	if len(text) <= 11 and len(text) >= 6:
		try:
			...
			await message.answer(f'Теперь оплатите нужную сумму!\nПосле успешной оплаты пользователь #{sec_user_id} получит приглашение, перейдя по которому он сможет загрузить товар', reply_markup=kb.paymentsMenu)
		except Exception as e:
			await message.answer('Вы ввели некорректный ID')
	else:
		await message.answer('Вы ввели некорректный ID или  произошла ошибка, нажмите /start для возврата')
